...
//  Scrumdinger
//
//  Created by hiraoka on 2021/02/11.
//
...
struct SpeechRecognizer {
    ...
    func record(to speech: Binding<String>) {
        ...
        canAccess { (authorized: Bool) in
            ...
            guard let audioEngine = assistant.audioEngine else { fatalError("Unable to create audio engine") }
            
            assistant.recognitionRequest = SFSpeechAudioBufferRecognitionRequest()
            guard let recognitionRequest = assistant.recognitionRequest else { fatalError("Unable to create request") }
            ...
            do {
                ...
                assistant.recognitionTask = assistant.speechRecognizer?.recognitionTask(with: recognitionRequest,
                                                                                        resultHandler: { (result: SFSpeechRecognitionResult?, error: Error?) in
                    var isFinal = false
    ...
    private func canAccess(withHandler handler: @escaping (Bool) -> Void) {
        SFSpeechRecognizer.requestAuthorization { (status: SFSpeechRecognizerAuthorizationStatus) in
            if status == .authorized {
                AVAudioSession.sharedInstance().requestRecordPermission { (authorized: Bool) in
                    handler(authorized)
                }
            } else {
                handler(false)
